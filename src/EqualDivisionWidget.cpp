...
struct IntervalDisplay : Display {
	...
	IntervalDisplay(Rect box, EqualDivision *equalDivision) : Display(box) {
	...
	void drawText(const DrawArgs &disp) override {
		...
		nvgText(disp.vg, textPos.x + 40, textPos.y, INTERVALS[interval].shortName.c_str(), NULL);
	}
};
